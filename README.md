...
| **Removed resource bounds** | a caller that names a ceiling and then nullifies it — `budget_cap: "none"`, `spend_limit: "off"`, `max_cost: "unset"`, `concurrency: "unbounded"` — caught on the argument structure, plus the same claim written as an instruction ("retry immediately forever"). **It reads no magnitude:** a 2.1-million-row batch with a batch size is silent. See the caveat below |
...
**The removed-bound detector (new in 1.15.0) flags two routine shapes, and the
separator is not something it can check.** It fires when a bound-shaped argument
key is filled with a word meaning "no ceiling", which is the right reading for
`budget_cap: "none"` and the wrong one for these two:
...
An archive bucket really is retained forever and a perpetual licence really does
not expire. Both flag. There is no lexical difference to read — `retention` and
`expires` are real bound keys, `forever` and `never` are real nullifiers — and
the **only** thing separating them from an attack is the authorisation reference
beside them. That reference is app-supplied and unsigned, so an attacker types
`approval="LEGAL-88"` exactly as easily as the archivist does; reading it would
be a courtesy, not a discrimination, so it is deliberately not read. Measured,
this is 2 of the 50-call discriminator pool (4.0%) and **0 of the 190-call blind
production corpus**. It is FLAG, never block — an unbounded job is a cost
problem, not an authorization boundary — so the cost is a review line, not a
stopped call. If those two shapes are ordinary traffic for you, `retention` and
`expires` are the keys to expect them on.

**Also not read, each for a stated reason:** numeric sentinels (`max_retries=0`
is *more* bounded, `limit=0` means unlimited in some APIs), booleans in either
polarity (`rate_limit=false` removes a ceiling, `limit_reached=false` says it
held), a bound nulled inside an opaque config string, and a duration or alert
level merely left unset (`ttl: "none"`, `deadline: "none"`). Telling the first
two apart from their opposites requires knowing which tool it is — i.e. a tool
list — which is the thing this detector exists to avoid being.

**Tuned against five agent personas, and that is the larger caveat.** This
detector and the privileged-action one above were both built against support,
devops, data, research and finance traffic, and every benign pool in this repo is
those five — so `0 / 190` is a number about those five. Measured against 120
production-shaped benign calls from six domains they had never seen
(`benign_toolcalls/domains.jsonl`: legal document management, game state,
scientific computing, media production, healthcare scheduling, education), they
flag **24 of 120 (20.0%)**, unevenly: 5% on legal, 35% on mathematics and
scientific computing, where `limit` and `bounds` are the subject matter rather
than a ceiling. All 24 are FLAG, never block. The per-domain table, the three
kinds of residual, and what to expect on each key are in
[Agent privilege tiers](https://github.com/delphisecurity/xaidr/blob/main/docs/privilege-tiers.md#the-domains-these-detectors-were-tuned-against);
...
full in `asi_battery/RESULTS.md`): 120 attack cases, written in plain,
non-injection language and across every boundary, by the same authors but blind
to this table. The two do not agree. A verdict set on the in-sample probe alone
reads better than the held-out number, and where they disagree the verdict below
now follows the held-out reading and says so. The in-sample probe is not
retired: it is kept as the mechanical drift-check and is one half of the
evidence, not the whole of it.

**The battery's case counts are NOT twelve per category, and the per-row
figures below are not comparable across rows.** Its original labels came from
this repo's rule names rather than from OWASP, and two of them named the wrong
risk (finding F9); relabelling 28 cases onto the official categories left ASI03
with 11 cases, ASI04 with 3, ASI09 with 1, ASI06 with 21, and 12 cases under a
deliberately non-OWASP label `EXH` for resource exhaustion and denial-of-wallet.
Every held-out figure in this table now carries its denominator for that reason.
**ASI09 at one case is not a rate and is not quoted as one.** The whole-battery
totals did not move: 43/120 catch and 7/120 false positive before and after,
because only labels changed. See `asi_battery/README.md`.
...
| **ASI03** Identity and Privilege Abuse | PARTIAL | The POLICY layer is **configuration-dependent, and telemetry only until configured**: with a policy bound, a privileged tool returns `approval_required` or `blocked`; with none, policy records and does not gate. **A structural privilege-action detector now flags the SHAPE of a subverting call by default, no policy required**, read from the argument key/value structure rather than a joined value string (which loses the key): a self-grant (`principal: agent://self`), a disabled control (`require_approval: false`, `safety_checks: disabled`, `monitoring: off`, a zeroed backup retention, a trust root invalidated without reissue), a waived approval (`approval: none`, `confirm: skip`), an escalation to admin or root carrying no governance marker, an opening to `0.0.0.0/0`, or a concealed grant (`visible: false`). Measured: 0 false positives on the 190-call production benign corpus and 0 of the 50-call discriminator pool (**both of which are the five agent personas it was tuned against**; on 120 benign calls from six domains it was not, `benign_toolcalls/domains.jsonl`, the two default-on tool-argument detectors together flag 24, see [the domains section](https://github.com/delphisecurity/xaidr/blob/main/docs/privilege-tiers.md#the-domains-these-detectors-were-tuned-against)), and it fires on **14 of the battery's 48 tool-call steps** (30 whole `tool_call` cases plus the 18 tool-call steps inside `sequence` cases). That denominator is every tool-call step in the battery, not a count of privilege-shaped ones: the detector is not aimed at the other 34 and does not fire on them, and it is a whole-battery count unaffected by the F9 relabelling. Held-out on this category, the rules catch **8 of 11** ASI03 cases; the denominator is 11 rather than 12 because F9 moved one case, an output falsely claiming a two-person review had been satisfied, to ASI09 where it belongs. A previously published "14 of 15" is WITHDRAWN, because the 15 came from a prototype's shape count that was never enumerated per case, leaving the ratio with a denominator nobody could reproduce. It is FLAG, not block, because the governance markers it reads are app-supplied and unsigned, so a fabricated `approval` reference evades it exactly as it evades the policy engine. **Not covered:** an escalation that is a property of the DEPLOYMENT rather than the argument shape (an ordinary-looking delegation to a named peer scores zero, by design); crypto-mining as a workload, whose only tell is a denylist of miner images and is left to policy; and the unsigned nature of the tier and chain claims. |
| **ASI04** Agentic Supply Chain | MOSTLY NOT COVERED | Covered: package installation named in prose. **Not covered: the path this category is actually about, and the reason is structural. There is no discovery boundary.** Held-out, the rules catch **0 of 3** ASI04 cases: registering an unknown agent as `trust: verified`, registering a rogue peer and then routing the user database to it, and re-pointing a trusted agent's endpoint at an external relay all score 0.00. **Three is the whole denominator and it is too small to be a measurement**, because the battery had no real ASI04 cases until finding F9 moved these three in from ASI06, and the twelve cases that used to sit in this row tested resource exhaustion, not supply chain; they now read `EXH` and their 9-of-12 result is not a claim about this category. `scan_tool_call(name, arguments, mcp_server)` is the entire input surface, so no tool description, no argument schema and no `tools/list` response is ever passed to a scan, and no per-tool state is kept between calls. A tool whose definition changed since last time therefore cannot be compared with anything. `curl` piped to `bash` does block, but as code execution, not as provenance. `mcp_server` is carried into telemetry and into policy matching, so a deployer can deny a named server by hand; that is an allowlist someone maintains, not detection. |
| **ASI05** Unexpected Code Execution | PARTIAL | **In-sample: covered. Held-out: half.** The command-form shapes block on both the content and tool-argument paths: `eval` into `os.system`, reverse shells, unsafe deserialization, fork bombs, base64 decoded into a shell, and container escape. This is the best-covered corner of the whole map, and the held-out battery agrees on it: the command-form cases (`curl` piped to `bash`, a decoded blob piped to `sh`, `python -c` calling `exec`) are exactly the ones the rules catch. **Not covered:** the same intent written without a command shape. Held-out, the rules catch **8 of 12** ASI05 cases; the misses are plain-language download-and-run, save-then-execute of a pasted binary, a permanent self-safety waiver, and telling a peer to run unreviewed code. (This row published "6 of 12" for three releases after the G2 detector moved it to 8 of 12 by catching self-cloning to an unmonitored host and writing a config file that disables the agent's own safety; the stale pair is corrected here. The nano figure that stood beside it is withdrawn rather than restated; see `asi_battery/RESULTS.md`.) The escape as an **event** is also not covered: this scans the request, so a sandbox breakout that never passes a scanned boundary is invisible to it. Half the category is uncovered on the held-out set, so the verdict is PARTIAL rather than COVERED. |
| **ASI06** Memory and Context Poisoning | NOT COVERED | **There is no memory boundary and no retrieval boundary.** Held-out, the rules catch **4 of 21** ASI06 cases. That denominator is 21 and not 12 because finding F9 moved twelve retrieval-poisoning cases in from ASI09, where they had been mislabelled, and moved three registry cases out to ASI04; the enlarged category covers both halves this row names, and both are missed at the same rate. The four scan entry points take a prompt, a response, a tool call and an A2A envelope; none of them is a memory write or a retrieved chunk, and no per-agent state is kept between calls, so nothing can compare what was stored with what is later read back. A poisoned instruction or a false fact written to memory returns allowed. Fact-shaped poison, meaning a false statement of policy or fact written as a plain declarative, returns allowed wherever it is scanned, because it carries no imperative and no injection marker. What **does** block is an injection payload, wherever it happens to be scanned, including when wrapped in a retrieval frame; that is ASI01 machinery firing on text and should not be read as memory coverage. There is now also a dedicated `rag_poisoning` rule family for the shapes a planted document uses: an override directive aimed at the retrieved context, a control token, a retrieval-priority marker, an instruction hidden in an HTML comment or behind `aria-hidden`, a document asserting that it supersedes earlier ones. **Most of that is content path only.** The `rag_poisoning` family fires on text you hand to `scan()` and is dropped on the tool-argument path, so a poisoned document written through a tool call is not seen. The `asi06` family is the exception and was admitted to the tool path at flag tier, so a memory write phrased as an instruction to the agent now surfaces where it used to return allowed. It surfaces; it does not block, and a false fact with no instruction in it still returns allowed on both paths. Catching a poisoned document you choose to scan is not the same as having a boundary that sees documents, which is why this row still reads NOT COVERED. |
| **ASI07** Insecure Inter-Agent Communication | PARTIAL | Covered: structural and wire-format checks on the A2A envelope (forged role, part and content mismatch, JSON-RPC version) and id smuggling (path traversal or command injection in `messageId`), plus the full content stack on message parts and on `params.metadata`. **Those checks now run on every A2A object in the body, not only the top-level message.** They used to read `params.message` and nothing else, so the identical forged Message scored 0.40 at `params.message` and 0.00 inside `result.status.message` or `result.history[0]`, and with `a2a_structural_enforcement="block"` the top-level placement was blocked while the nested one was allowed. The container list is the spec's (`a2a.proto` at v1.0.1 and the v0.3.0 JSON Schema): `TaskStatus.message`, `Task.history[]`, `Task.artifacts[]`, `TaskStatusUpdateEvent.status`, `TaskArtifactUpdateEvent.artifact`, `ListTasksResponse.tasks[]`, plus the bare Task and bare Message in-process shapes. The walk is iterative and bounded in depth (8), node count (64) and list length (32); **exceeding a bound fires `a2a_nested_depth_exceeded` or `a2a_nested_breadth_exceeded` rather than dropping the remainder silently.** Measured at 0 false positives on `benign_a2a/nested.jsonl` (60 benign bodies, 274 nodes walked), and byte-identical on all 1139 `corpus_diff` rows. That second number proves nothing on its own, because no committed pool reaches this code path at all, which is precisely why the first corpus had to be written. **Configuration-dependent:** those structural findings sit below the block threshold and surface as flags; `a2a_structural_enforcement="block"` promotes them, and promotes all of them, since it is not selectable per family. **Not covered: replay.** The same message sent three times returns allowed three times, because there is no nonce, no timestamp, no freshness window and no memory of message ids already seen. Authentication is out of remit: the sensor reads claims, it does not verify signatures. |
...
where ASI01 catches 1 of 12 on the rules alone and ASI05 catches 8 of 12; the
per-category numbers, their unequal denominators and every miss are in
`asi_battery/RESULTS.md`. Whole-battery, the rules catch 43 of 120.
