...
# ---------------------------------------------------------------------------
# Commercial sink — NOT part of the open-source SDK.
# In the open repo this class is removed and StdoutReporter is the default.
# It is kept here in the commercial build so existing Brain delivery is
# preserved while the reporter seam is introduced. Moves to the paid
# `xaidr-brain` package on repo split.
# ---------------------------------------------------------------------------
class _BrainReporter:
    """Delivers event batches to the Delphi Brain (commercial). Implements the
    Reporter protocol; preserves the prior POST + optional request signing."""

    def __init__(self, sentinel_url: str, api_key: str, identity: Any = None) -> None:
        import httpx  # lazy
    ...
    def report(self, events: list[dict[str, Any]]) -> None:
        try:
            body_bytes = json.dumps({"events": events}, separators=(",", ":")).encode("utf-8")
            headers: dict[str, str] = {}
            if self._identity:
                sig_headers = self._identity.sign_request(
                    "POST", "/v1/events/batch", body_bytes
                )
            ...
            logger.warning("Brain delivery failed, dropping %d events: %s", len(events), exc)
